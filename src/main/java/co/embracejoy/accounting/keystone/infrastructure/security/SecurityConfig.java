 ...
 *
 * <p>{@code @Order(3)} — evaluated after {@code UiSecurityConfig}'s browser-facing chain
 * ({@code @Order(2)}), which claims {@code /admin/ui/**} and the OAuth2 handshake paths, and after
 * the embedded SAS's chains ({@code @Order(0)}/{@code @Order(1)}, {@code dev}/{@code test} only).
 * This chain's {@code anyRequest()} match picks up everything else, i.e. the bearer-JWT API
 * surface.
 *

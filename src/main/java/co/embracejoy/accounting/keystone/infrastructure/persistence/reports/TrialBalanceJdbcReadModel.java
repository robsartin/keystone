 ...
 * filtered by {@code p.tenant_id = :tenantId} and {@code je.occurred_on <= :asOf} and (optionally)
 * {@code balance != 0}. Uses Spring 6.1+ {@link JdbcClient} (the modern fluent client) with named
 * parameters — the {@code :includeZero} flag is referenced once in the {@code HAVING} clause and
 * Spring binds it for us.
 */
@Repository
...
public class TrialBalanceJdbcReadModel implements TrialBalanceReadModel {
  ...
  @Override
  public List<TrialBalanceRow> fetch(TenantId tenantId, LocalDate asOf, boolean includeZero) {
